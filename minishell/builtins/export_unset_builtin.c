...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   export_unset_builtin.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yofouzi <yofouzi@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/14 13:31:55 by yofouzi           #+#    #+#             */
/*   Updated: 2025/08/14 13:34:58 by yofouzi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	export_builtin(t_data *data, char **args)
{
	...
	i = 1;
	if (!args[i])
		return (env_builtin_export(data, NULL));
	while (args[i])
	{
		if (!is_valid_env_var_key(args[i]))
		{
			error_cmd("export", args[i], "not a valid identifier", false);
			ret = EXIT_FAILURE;
		}
...
int	unset_builtin(t_data *data, char **args)
{
	...
	while (args[i])
	{
		...
		{
			error_cmd("unset", args[i], "not a valid identifier", false);
			ret = EXIT_FAILURE;
		}
		else
		{

...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/14 13:31:18 by yofouzi           #+#    #+#             */
/*   Updated: 2025/08/14 13:34:21 by yofouzi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	bool	chdir_errno_mod(char *path)
{
	if (errno == ESTALE)
		errno = ENOENT;
	error_cmd("cd", path, strerror(errno), errno);
	return (false);
}
...
int	cd_builtin(t_data *data, char **args)
{
		...
		|| args[1][0] == '\0' || ft_strncmp(args[1], "--", 3) == 0)
	{
		...
		if (!path || *path == '\0' || ft_isspace(*path))
			return (error_cmd("cd", NULL, "HOME not set", EXIT_FAILURE));
		return (!change_dir(data, path));
	}
	if (args[2])
		return (error_cmd("cd", NULL, "too many arguments", EXIT_FAILURE));
	if (ft_strncmp(args[1], "-", 2) == 0)
	{
		path = get_env_var_value(data->env, "OLDPWD");
		if (!path)
			return (error_cmd("cd", NULL, "OLDPWD not set", EXIT_FAILURE));
		return (!change_dir(data, path));
	}

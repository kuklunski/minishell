...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/14 13:32:33 by yofouzi           #+#    #+#             */
/*   Updated: 2025/08/14 13:35:37 by yofouzi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	execute_sys_bin(t_data *data, t_command *cmd)
{
	...
	if (execve(cmd->path, cmd->args, data->env) == -1)
		error_cmd("execve", NULL, strerror(errno), errno);
	return (EXIT_FAILURE);
}
...
static int	execute_local_bin(t_data *data, t_command *cmd)
{
	...
	if (execve(cmd->command, cmd->args, data->env) == -1)
		return (error_cmd("execve", NULL, strerror(errno), errno));
	return (EXIT_FAILURE);
}
